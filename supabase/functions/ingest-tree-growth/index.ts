...
Deno.serve(async (req) => {
  ...
  if (!apiKey) {
    return jsonResponse({
      error: 'Missing access key. Send header: x-api-key: ta_...',
    }, 401);
  }
  ...
  const measurementDate = String(body.measurement_date || '').trim();
  if (!measurementDate) {
    return jsonResponse({ error: 'measurement_date is required' }, 400);
  }

  const readings: Record<string, number | null> = {};
  let hasMeasurement = false;
  for (const key of GROWTH_FIELD_KEYS) {
    const value = parseNumber(body[key]);
    readings[key] = value;
    if (value != null) hasMeasurement = true;
  }

  if (!hasMeasurement) {
    return jsonResponse({ error: 'Provide at least one measurement value' }, 400);
  }
